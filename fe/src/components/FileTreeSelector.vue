...
const fetchFiles = async () => {
  if (!props.cloudToken) return
  
  try {
    loading.value = true
    const response = await storageBridgeApi.getPersonNodes({
...
// 监听 cloudToken 变化
watch(() => props.cloudToken, (newToken) => {
  if (newToken) {
    fetchFiles()
  } else {
    files.value = []
...
const loadChildren = async (parentFile: TreeFileNode) => {
  if (!props.cloudToken || loadingIds.value.has(parentFile.id)) return
