...
- 提供 Web 界面进行文件浏览和管理
- 多线程流式加速浏览视频
- **媒体目录映射**：将云盘文件通过strm形式映射到本地media_dir目录，完美兼容Emby、Jellyfin、Plex等媒体服务器 ⚠️ *测试功能*

> ⚠️ **重要提示**：媒体目录映射功能目前处于测试阶段，需要使用alpha版本的Docker镜像，使用过程中如遇到问题请及时通过 GitHub Issue 反馈。
>
...
docker run -d \
  ...
  -v $(pwd)/media_dir:/app/media_dir \
  --restart unless-stopped \
  docker.1ms.run/xxcheng123/cloudpan189-share:alpha
```
...
# 或拉取测试版本（包含媒体目录映射功能）
docker pull xxcheng123/cloudpan189-share:alpha
